/**
 * ISONormalizer — the single place in the codebase that resolves ISO codes.
 *
 * RULE: Import and call normalizeISO() everywhere you handle country codes.
 * Never write inline normalization logic. This is what killed V1.
 *
 * Supports: alpha-2 → alpha-3, common aliases, case-insensitive input.
 * Returns: 3-letter uppercase ISO 3166-1 alpha-3, or null if unresolvable.
 */
...
/**
 * Normalize any country identifier to ISO 3166-1 alpha-3.
 * @param {string | null | undefined} input
 * @returns {string | null}
 */
export function normalizeISO(input) {
  if (!input) return null;
  const clean = String(input).trim().toUpperCase().replace(/[^A-Z]/g, '');
  if (!clean) return null;
  // Already alpha-3 and valid
  if (clean.length === 3 && Object.values(LOOKUP).includes(clean)) return clean;
