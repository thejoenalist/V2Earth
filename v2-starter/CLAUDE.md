...
| M2 | ✅ Complete | Chat → SimulationCommand flow with API proxy |
| M3 | ✅ Complete | Real CMIP6 data in pipeline, temperature layer live |
| M4 | 🔧 In progress | 6 "ready" event renders — hurricane done, 5 remaining |
...
1. ~~**Supabase Edge Function**~~ ✅ `supabase/functions/parse-scenario/index.ts` — implemented.
2. ~~**Python pipeline fetchers**~~ ✅ `fetch_cmip6.py`, `fetch_worldbank.py`, `fetch_boundaries.py` — all implemented; data baked and validated.
3. ~~**Baked JSON schema definition**~~ ✅ `pipeline/SCHEMA.md` exists.
4. **Layer implementations** — `TemperatureLayer` done (M3). `SeaLevelLayer` and others are M4 work.
5. **Privacy policy + Terms of Service** — required before any public traffic.
...
11. ~~**`#chat-eject-btn` in index.html**~~ ✅ Button exists at line 382 and is wired in ChatInterface.js.
